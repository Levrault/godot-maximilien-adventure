...
func modulate_letter(label: Label, found: bool) -> void:
	if found:
		$Tween.interpolate_property(label, "modulate", label.modulate, Color(1,1,1,1), .5, Tween.TRANS_LINEAR, Tween.TRANS_LINEAR)
	else:
		$Tween.interpolate_property(label, "modulate", label.modulate, Color(1,1,1,.15), .5, Tween.TRANS_LINEAR, Tween.TRANS_LINEAR)
	$Tween.start()
